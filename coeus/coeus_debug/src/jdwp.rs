...
pub struct JdwpClient {
    ...
    frame_id_size: u32,
    pending_events: VecDeque<JdwpCommandPacket>,
    pending_replies: VecDeque<JdwpReplyPacket>,
}

const JDWP_HANDSHAKE: &[u8] = b"JDWP-Handshake";

async fn connect_and_handshake(addr: &str, port: u16) -> anyhow::Result<TcpStream> {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};

    let mut tcp_handle = tokio::time::timeout(
        Duration::from_secs(10),
        TcpStream::connect(format!("{addr}:{port}")),
    )
    .await
    .map_err(|_| anyhow::anyhow!("JDWP connection timed out after 10 seconds"))??;

    tokio::time::timeout(Duration::from_secs(10), async {
        tcp_handle
            .write_all(JDWP_HANDSHAKE)
            .await
            .context("could not send the JDWP handshake")?;
        let mut handshake_answer = [0u8; JDWP_HANDSHAKE.len()];
        tcp_handle
            .read_exact(&mut handshake_answer)
            .await
            .context("JDWP peer closed the connection during the handshake")?;
        if handshake_answer != JDWP_HANDSHAKE {
            bail!("wrong JDWP handshake answer");
        }
        Ok::<_, anyhow::Error>(tcp_handle)
    })
    .await
    .map_err(|_| anyhow::anyhow!("JDWP handshake timed out after 10 seconds"))?
}
...
// Private interface
impl JdwpClient {
    pub(crate) async fn wait_for_reply(&mut self) -> anyhow::Result<JdwpReplyPacket> {
        if let Some(reply) = self.pending_replies.pop_front() {
            return Ok(reply);
        }
        loop {
            let Some(packet) = self.wait_for_package().await else {
                bail!("No reply from the JDWP target");
            };
            match packet {
                JdwpPacket::ReplyPacket(reply) => return Ok(reply),
                JdwpPacket::CommandPacket(event) => self.pending_events.push_back(event),
            }
        }
    }

    async fn wait_for_event(&mut self) -> Option<JdwpCommandPacket> {
        ...
        loop {
            ...
            match packet {
                JdwpPacket::CommandPacket(event) => return Some(event),
                JdwpPacket::ReplyPacket(reply) => self.pending_replies.push_back(reply),
            }
        }
    }

    async fn wait_for_event_timeout_inner(
        &mut self,
        timeout: Duration,
    ) -> anyhow::Result<Option<JdwpCommandPacket>> {
        if let Some(event) = self.pending_events.pop_front() {
            ...
            tokio::select! {
                packet = self.rx.recv() => {
                    let Some(packet) = packet else {
                        bail!("JDWP connection closed while waiting for an event");
                    };
                    match packet {
                        JdwpPacket::CommandPacket(event) => return Ok(Some(event)),
                        JdwpPacket::ReplyPacket(reply) => self.pending_replies.push_back(reply),
                    }
                }
                _ = &mut timer => return Ok(None),
            }
        }
    }
    ...
    async fn get_object_signature(&mut self, reference_type: u64) -> anyhow::Result<String> {
        ...
        log::debug!("{:?}", reply);
        let mut data_cursor = Cursor::new(reply.get_data());
        String::from_bytes(&mut data_cursor)
    }
...
impl JdwpClient {
    ...
    pub fn new(addr: &str, port: u16) -> anyhow::Result<(Self, Runtime)> {
        ...
        let rt = Builder::new_multi_thread()
            ...
            .build()
            .unwrap();
        let tcp_handle = rt.block_on(async {
            let mut last_error = None;
            for attempt in 0..3 {
                if attempt > 0 {
                    tokio::time::sleep(Duration::from_millis(250 * attempt)).await;
                }
                match connect_and_handshake(addr, port).await {
                    Ok(tcp_handle) => return Ok(tcp_handle),
                    Err(error) if is_retryable_connection_error(&error) && attempt < 2 => {
                        last_error = Some(error);
                    }
                    Err(error) => return Err(error),
                }
            }
            Err(last_error.unwrap_or_else(|| anyhow::anyhow!("JDWP connection failed")))
        })?;
        ...
        let read_thread = rt.spawn(async move {
            loop {
                ...
                if length < 11 {
                    log::error!("invalid JDWP packet length: {length}");
                    break;
                }
        ...
        let write_thread = rt.spawn(async move {
            loop {
                let Some(pkg) = writer_rx.recv().await else {
                    break;
                };
    ...
    /// Wait for an asynchronous VM event. This intentionally has no timeout;
    /// the Python binding runs it on a worker thread, so keeping the listener
    /// alive is what allows a breakpoint hit after an arbitrary amount of
    /// time to reach the GUI.
    pub fn wait_for_event_blocking(
        &mut self,
        runtime: &Runtime,
    ) -> Option<JdwpCommandPacket> {
        runtime.block_on(self.wait_for_event())
    }
    pub fn wait_for_event_timeout(
        ...
        runtime.block_on(self.wait_for_event_timeout_inner(timeout))
    }
    pub fn resume(&mut self, runtime: &Runtime, id: u32) -> anyhow::Result<()> {
        runtime.block_on(async {
            ...
            let _ = self.wait_for_reply().await?;
            Ok(())
        })
    }
    ...
    pub fn clear_breakpoint(&mut self, runtime: &Runtime, event_id: u32) -> anyhow::Result<()> {
        runtime.block_on(async {
            let breakpoint = JdwpCommandPacket::clear_breakpoint(rand::random(), event_id)?;
            self.send_cmd(JdwpPacket::CommandPacket(breakpoint))?;
            let reply = self.wait_for_reply().await?;
            if reply.is_error() {
                bail!("Clear breakpoint failed: {}", reply.get_error());
            }
            Ok(())
        })
    }
...
impl JdwpCommandPacket {
    ...
    pub fn clear_breakpoint(id: u32, event_id: u32) -> anyhow::Result<Self> {
        let mut data = vec![];
        data.write_u8(JdwpEventType::Breakpoint as u8)?;
        data.write_u32::<BigEndian>(event_id)?;
        Ok(Self {
